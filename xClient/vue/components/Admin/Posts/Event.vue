                        ...
                        >
                            ...
                            <template slot="form-top" v-if="formTopState.show">
                                <form class="admin-form-card user-form" @submit.prevent>
                                    <div class="user-form-title">Create new Event</div>
                                    ...
                                    <div class="layout-align-space-around-start layout-row">
                                        <div class="form-input-container flex" full>
                                            ...
                                            <Datetime
                                                ...
                                                value-zone="Asia/Vientiane"
                                                zone="Asia/Vientiane"
                                                v-model="models.formTop.end_event"
                                                input-id="end_event"
                                                :input-class="'admin-input-datepicker'"
                                            />
                                            <template v-if="validated().end_event">
                                                <div admin-messages>
                                                    <div admin-message class="message-required">{{ validated().end_event
                                                        }}
                                                    </div>
                                                </div>
                                            </template>
                                    ...
                                    <div class="layout-align-space-around-start layout-row">
                                        <AdminInput
                                            v-model="models.formTop.hosted_by"
                                            :validateText="validated().hosted_by"
                                            :label="'Hosted by *'"
                                            :inputType="'text'"
                                        />
                                    </div>
                            ...
                            <template slot-scope="{fireEvent, position, rowContent}" slot="form-row-detail">
                                ...
                                <div class="layout-align-space-around-start layout-row">
                                    <div class="form-input-container flex" full>
                                        <label>Start event date *</label>
                                        <Datetime
                                            v-model="rowContent.data.start_event"
                                            @onOpenPopup="onDateTimeOpen"
                                            @close="onDateTimeClose"
                                            value-zone="Asia/Vientiane"
                                            zone="Asia/Vientiane"
                                            ...
                                            :input-class="'admin-input-datepicker'"
                                        />
                                        <template v-if="rowContent.validated.start_event">
                                            <div admin-messages>
                                                <div
                                                    admin-message
                                                    class="message-required"
                                                >{{ rowContent.validated.start_event }}
                                                </div>
                                            </div>
                                        </template>
                                    </div>
                                ...
                                <div class="layout-align-space-around-start layout-row">
                                    <div class="form-input-container flex" full>
                                        <label>Close event date *</label>
                                        <Datetime
                                            v-model="rowContent.data.end_event"
                                            @onOpenPopup="onDateTimeOpen"
                                            @close="onDateTimeClose"
                                            value-zone="Asia/Vientiane"
                                            zone="Asia/Vientiane"
                                            format="dd-MM-yyyy HH:mm"
                                            type="datetime"
                                            :input-id="`event-date${rowContent.data.id}`"
                                            :input-class="'admin-input-datepicker'"
                                        />
                                        <template v-if="rowContent.validated.end_event">
                                            <div admin-messages>
                                                <div
                                                    admin-message
                                                    class="message-required"
                                                >{{ rowContent.validated.end_event }}
                                                </div>
                                            </div>
                                        </template>
                                    </div>
                                ...
                                <div class="layout-align-space-around-start layout-row">
                                    <AdminInput
                                        v-model="rowContent.data.hosted_by"
                                        :validateText="rowContent.validated.hosted_by"
                                        :label="'Hosted by *'"
                                        :inputType="'text'"
                                        @onInputEnter="edit(fireEvent, rowContent.data, position)"
                                    />
                                ...
                                <div
                                    ...
                                    <button v-if="rowContent.data.status === 'open'"
                                            @click="manageStatus(fireEvent, rowContent.data, position, 'event', 'close')"
                                    ...
                                    <button v-if="rowContent.data.status === 'pending'"
                                            @click="manageStatus(fireEvent, rowContent.data, position, 'event', 'approve')"
                                            class="v-md-button primary theme-blue">Approve
                                    </button>
                                    <button v-else-if="rowContent.data.status === 'close'"
                                            @click="manageStatus(fireEvent, rowContent.data, position, 'event', 'open')"
...
<script>
    ...
    export default AdminBase.extend({
        ...
        data: () => ({
            ...
            tabs: [{name: "Event"}],
            headers: [
                {class: "th-sortable", name: "Event name", width: "30%"},
        ...
        methods: {
            ...mapActions(["postCreateEvent", "postUpdateEvent", "postDeleteEvent"]),
                    ...
                    rows: [
                        ...
                        },
                        {
                            ...
                            modal: {
                                ...
                                active: false,
                                type: "warning",
                                message: `When you delete the event, the event will be permanently deleted and cannot be un-deleted.`,
                                action: {act: this.postDeleteEvent, text: "Delete"},
                                data: data
                            }
                        }
                    ]
            ...
            positiveAction() {
                ...
                if (action.act) {
                    ...
                    action
                        ...
                        .then(r => {
                            if (r.success) {
                                this.showInfoToast({msg: "The event was deleted!", dt});
                                this.getItems();
                            } else {
                                this.showErrorToast({
            ...
            add() {
                ...
                ft.loading = true;
                this.postCreateEvent(this.models.formTop)
                    .then(r => {
                        if (r.success) {
                            this.getItems();
                            ft.show = false;
            ...
            edit(fireEvent, data, position) {
                ...
                data.id = v.data.id;
                this.postUpdateEvent(data)
                    .then(r => {
                        if (r.success) {
                            this.showInfoToast({msg: "The event was updated!", dt});
                            this.getItems();
                        } else {
                            this.showErrorToast({
