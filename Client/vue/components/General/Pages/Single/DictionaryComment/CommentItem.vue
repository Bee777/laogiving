...
<script>
    export default {
        ...
        methods: {
            ...
            checkHashIds() {
                let localHash_ids = this.$utils.getLocalStorage('dictionary_comments_hash_ids');
                if (localHash_ids) {
                    let {hash_ids} = this.$utils.toJSON(localHash_ids);
                    this.user.hash_id = hash_ids.filter((hash) => {
                        return hash.indexOf(this.user.hash_id) !== -1;
                    }).shift();
                    this.owner = typeof this.user.hash_id !== 'undefined';
                }
            }
        },
        mounted() {
