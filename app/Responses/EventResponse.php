...
class EventResponse implements Responsable
{
    ...
    public function get($request)
    {
        ...
        $data = Posts::select($fields)->where('type','event');
        ...
        $data->map(function ($d) {
            ...
            $d->filename = $d->image;
            $d->start_event = Helpers::toFormatDateString($d->start_date, 'c'); //$d->start_date->format('c');//covert timestamp to ISO 8601 format.
            $d->end_event = Helpers::toFormatDateString($d->deadline, 'c'); //$d->deadline->format('c');//covert timestamp to ISO 8601 format.
