...
class UserEventResponse implements Responsable
{
    ...
    {
        $fields = ['id', 'title', 'place', 'status', 'hosted_by', 'start_date', 'deadline', 'image', 'description', 'user_id', 'created_at', 'updated_at'];
        $request->request->add(['fields' => $fields]);
        $text = $this->options['text'];
        $paginateLimit = $this->options['limit'];
        $data = Posts::select($fields)->where('type', 'event')->where('user_id', auth()->user()->id);
        ...
        $data->map(function ($d) {
            ...
            $d->filename = $d->image;
            $d->start_event = Helpers::toFormatDateString($d->start_date); //$d->start_date->format('c');//covert timestamp to ISO 8601 format.
            $d->end_event = Helpers::toFormatDateString($d->deadline); //$d->deadline->format('c');//covert timestamp to ISO 8601 format.
    ...
    public function toResponse($request)
    {
        ...
        if (Helpers::isAjax($request)) {
            if ($this->actionType === 'insert') {
                ...
                $info->hosted_by = $request->get('hosted_by');
                $info->type = 'event';
                $info->status = 'pending';
