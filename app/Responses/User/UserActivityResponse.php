...
class UserActivityResponse implements Responsable
{
    ...
    {
        $fields = ['id', 'title', 'place', 'image', 'description', 'status', 'start_date', 'user_id', 'created_at', 'updated_at'];
        $request->request->add(['fields' => $fields]);
        $text = $this->options['text'];
        $paginateLimit = $this->options['limit'];
        $data = Posts::select($fields)->where('type', 'activity')->where('user_id', auth()->user()->id);
