...
class VolunteeringActivity extends Model
{
    ...
    public static function mapPosition($activity_id)
    {
        $selfActivity = self::find($activity_id);
        $mPositions = [
            [
                'collapsed' => false,
                'id' => 0,
        ...
        if (isset($selfActivity)) {
            $positions = $selfActivity->positions;
            if (count($positions) > 0) {
                $mPositions = [];
            }
            foreach ($positions as $position) {
