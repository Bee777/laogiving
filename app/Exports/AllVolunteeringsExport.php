...
class AllVolunteeringsExport extends DefaultValueBinder implements FromView, WithTitle, WithCustomValueBinder, ShouldAutoSize, WithEvents
{
    protected $data;
    ...
    {
        return view('generate.users.all-volunteering-signup-excel', $this->data);
    }
    ...
    public function registerEvents(): array
    {
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                //set content style
                $start_row = 8;
                $records = $this->data['sign_up_activities'];
                foreach ($records as $coord => $item) {
                    $event->sheet->styleCells(
                        'A' . ($start_row + $coord) . ':R' . ($start_row + $coord),
                        [
                            'alignment' => [
                                'vertical' => \PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER,
                                'horizontal' => \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_LEFT,
